                        ...
                        google.script.run
                            .withSuccessHandler((res) => {
                                ...
                                if (res.success) {
                                    ...
                                    // MENSAJE CORREGIDO Y PROTOCOLO NATIVO
        ...
        <div className="p-4 md:p-8 bg-white min-h-full overflow-y-auto">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 print:hidden">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => window.print()} className="bg-[var(--color-primary)] text-white p-3 rounded-xl shadow-md"><Icon name="printer" size={18}/></button>
                </div>
            </header>
            ...
            <div className="bg-white p-6 border-b border-gray-100 mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden">
                <div><label className="text-[10px] font-bold uppercase text-gray-500 mb-2 block">Período</label>
                    <select className="w-full border border-gray-200 p-3 rounded-xl font-bold text-gray-900 outline-none" value={dateRange} onChange={e => setDateRange(e.target.value)}><option value="today">Hoy</option><option value="week">Semana</option><option value="month">Mes</option><option value="all">Todo</option></select>
                </div>
                <div><label className="text-[10px] font-bold uppercase text-gray-500 mb-2 block">Profesional</label>
                    <select className="w-full border border-gray-200 p-3 rounded-xl font-bold text-gray-900 outline-none" value={profFilter} onChange={e => setProfFilter(e.target.value)}><option value="ALL">Todos</option>{professionals.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
                </div>
                <div><label className="text-[10px] font-bold uppercase text-gray-500 mb-2 block">Medio de Pago</label>
                    <select className="w-full border border-gray-200 p-3 rounded-xl font-bold text-gray-900 outline-none" value={paymentFilter} onChange={e => setPaymentFilter(e.target.value)}><option value="ALL">Todos</option><option value="cash">Efectivo</option><option value="transfer">Transferencia</option></select>
                </div>
                <div><label className="text-[10px] font-bold uppercase text-gray-500 mb-2 block">Cliente</label>
                    <input type="text" placeholder="Buscar..." className="w-full border border-gray-200 p-3 rounded-xl font-bold text-gray-900 outline-none" value={searchClient} onChange={e => setSearchClient(e.target.value)}/>
                </div>
            </div>

            {/* ÁREA DE REPORTE: BLANCO TOTAL + RECUADRO SUTIL DEL COLOR DE MARCA */}
            <div id="report-area" className="w-full bg-white p-8 md:p-12 rounded-[2.5rem] border-2 border-[var(--color-primary)] shadow-sm print:p-0 print:border-0">
                
                {/* CABECERA */}
                <div className="flex justify-between items-start border-b border-gray-100 pb-10 mb-10">
                    <div className="flex items-center gap-6">
                        {localLogo ? <img src={localLogo} className="h-20 w-auto object-contain" /> : <div className="w-16 h-16 bg-gray-50 rounded-2xl flex items-center justify-center font-black text-gray-400 text-2xl">S</div>}
                        <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                    ...
                    <div className="p-6 bg-white rounded-2xl border-2 border-green-500/20">
                ...
                {/* TABLA: BLANCO Y NEGRO PURO */}
                <div className="space-y-4">
                    <div className="flex items-center gap-3 border-l-4 border-[var(--color-primary)] pl-4 mb-6">
                        <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest">Detalle de Cobranzas y Comisiones</h3>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-left bg-white">
                            <thead>
                                <tr className="text-[9px] uppercase text-gray-500 font-bold border-b border-gray-300">
                            ...
                            <tbody className="bg-white">
                                {tableData.length === 0 ? (
                                    <tr><td colSpan="6" className="py-10 text-center text-gray-400 italic text-xs">Sin registros.</td></tr>
                                ) : (
                                    tableData.map(row => (
                                        <tr key={row.id} className="text-[11px] border-b border-gray-100 bg-white">
                                            <td className="py-4 px-2 text-gray-900 font-medium whitespace-nowrap leading-tight">
                                                {new Date(row.date).toLocaleDateString('es-ES', {day:'2-digit', month:'2-digit'})}<br/>
                                                <span className="text-[10px] text-gray-500 font-normal">{new Date(row.date).toLocaleTimeString('es-ES', {hour:'2-digit', minute:'2-digit'})} hs</span>
                                            </td>
                                            <td className="py-4 px-2">
                                                <p className="font-bold text-gray-900">{row.clientName}</p>
                                                <p className="text-[10px] text-gray-600">{row.treatmentName}</p>
                                            </td>
                                            <td className="py-4 px-2">
                                            ...
                                            <td className="py-4 px-2 text-right font-bold text-gray-900">${row.collectedAmount.toLocaleString()}</td>
                                            <td className="py-4 px-2 text-right text-gray-900 font-medium">{row.rate}%</td>
                                            <td className="py-4 px-2 text-right font-black text-gray-900">${row.commissionAmount.toLocaleString()}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                            <tfoot className="bg-white border-t-2 border-black">
                                <tr>
                                    <td colSpan="5" className="py-6 px-4 text-right text-[10px] font-bold uppercase text-gray-500">Total a Liquidar:</td>
                                    <td className="py-6 px-4 text-right text-xl font-black text-black">${totalComisiones.toLocaleString()}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
